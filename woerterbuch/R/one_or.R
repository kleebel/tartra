...
#' @export
oneor_apply_search <- function(corpus, condition) {
  ...
  relevant_documents <- foreach(
    ...
    doc <- corpus[[doc_id]]
    found_words <- oneor_term_search(doc, condition)
...
#' @export
oneor_term_search <- function(doc, condition) {
  if (!is.list(condition) || !"OR" %in% names(condition)) {
    stop("Condition muss eine Liste mit einem OR-Schluessel sein.")
  }

  # Ueberpruefe die OR-Bedingung
  or_terms <- condition$OR
